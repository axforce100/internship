...
osd_model_path = 'Models/CRNN_OSD_MODEL.pth'
...
def split_audio_by_labels(waveform, sr, frame_times, speaker_labels, osd_labels, audio_path):
    # Extract base name of audio file without extension
    audio_name = os.path.splitext(os.path.basename(audio_path))[0]
    output_dir = os.path.join("diarization_result", audio_name)
    
    os.makedirs(output_dir, exist_ok=True)

    frame_starts = [start for start, _ in frame_times]
    frame_ends = [end for _, end in frame_times]
    ...
    for i in range(len(speaker_labels)):
        ...
        is_overlap = osd_labels[i]

        category = 'overlap' if is_overlap else f'speaker_{label}'

        if category != current_label or is_overlap != current_osd:
            if current_label is not None:
                segment_end = frame_starts[i]
                segment_waveform = waveform[:, segment_start:segment_end]
                duration = (segment_end - segment_start) / sr

                if duration >= 0.1:
                    filename = f"{current_label}_segment_{segment_counters[current_label]}.wav"
                    filepath = os.path.join(output_dir, filename)

                    torchaudio.save(
                        filepath,
                        segment_waveform.cpu(),
                        sample_rate=sr,
                        bits_per_sample=16
                    )
                    print(f"Saved {filename} ({duration:.2f}s)")
                    segment_counters[current_label] += 1

            segment_start = frame_starts[i]
            current_label = category
            current_osd = is_overlap

    segment_waveform = waveform[:, segment_start:frame_ends[-1]]
    duration = (frame_ends[-1] - segment_start) / sr
...
if __name__ == "__main__":
    ...
    wav_path = 'data/testing_audio/test.wav'
    ...
    for segment in speech_timestamps:
        ...
        for i, frame in enumerate(frames):
            ...
            all_embeddings.append(embedding)  # Flatten to 1D
            ...
            with torch.no_grad():
                osd_output = osd_model(mel_spec)  # shape: [1, T, 1]
                # print(osd_output)
                osd_probs_frame = torch.sigmoid(osd_output.squeeze()).cpu().numpy()  # shape: [T]
                # print(osd_probs_frame)

                # Threshold each timestep
                timestep_preds = (osd_probs_frame >= osd_threshold).astype(int)

                # Aggregate decision: if >30% of frame is overlapping
                overlap_ratio = np.mean(timestep_preds)
                osd_pred = int(overlap_ratio >= 0.5)

                osd_probs.append(overlap_ratio)
    ...
    smoothened_labels = median_filter(speaker_labels, size=3)
    ...
    for i in range(1, len(smoothened_labels)):
        ...
        if current_label != prev_label or current_osd != prev_osd:
            ...
            print(f"{start_time / sr:.2f}s - {end_time / sr:.2f}s | Speaker {prev_label} | Overlap: {bool(prev_osd)} | Score: {prev_prob:.2f}")
    ...
    print(f"{start_time / sr:.2f}s - {end_time / sr:.2f}s | Speaker {prev_label} | Overlap: {bool(prev_osd)} | Score: {prev_prob:.2f}")
    # print("Model weights std:", osd_model.fc.weight.std().item())

    print("\nSplitting audio into speaker segments...")
    split_audio_by_labels(
        waveform,
        sr,
        frame_times,
        smoothened_labels,
        osd_labels,
        audio_path=wav_path  # full path to input .wav file
    )
