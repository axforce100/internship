...
def main(args, hp):
    with torch.no_grad():
        ...
        chkpt_model = torch.load(args.checkpoint_path,weights_only=False)['model']
        ...
        # Get base name of the input mixed file
        base_name = os.path.splitext(os.path.basename(args.mixed_file))[0]

        # Add a timestamp to make it unique per run
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

        # Create unique output path
        out_path = os.path.join(args.out_dir, f'{base_name}_{timestamp}.wav')
