...
def vad_merge(w): #optional (not used for data generation)
...
def mix_dual(hp, args, audio, base_num, s1_dvec, s1_target, s2_dvec, s2_target, train=True):
    ...
    dir_ = os.path.join(args.out_dir, 'train8' if train else 'test_unseen')
    ...
    sf.write(target_wav_path_2, w2, srate)
    target_mag_2, _ = audio.wav2spec(w2)
...
def robust_train_generate(hp, args, audio, train_spk, total_samples=50000):
    ...
    while base_num < total_mixtures and trials < max_trials:
        ...
        try:
            ...
            result = mix_dual(hp, args, audio, base_num, s1_dvec, s1_target, s2_dvec, s2_target, train=True)
            if result != "skipped":
                base_num += 1
                pbar.update(1)
...
if __name__ == '__main__':
    ...
    os.makedirs(os.path.join(args.out_dir, 'train8'), exist_ok=True)
    ...
    if args.libri_dir is not None:
        train_folders = [x for x in glob.glob(os.path.join(args.libri_dir, 'train/train-clean-100', '*'))
                         if os.path.isdir(x)] + \
                        [x for x in glob.glob(os.path.join(args.libri_dir, 'train/train-clean-360', '*'))
                         if os.path.isdir(x)]
    ...
    robust_train_generate(hp, args, audio, train_spk, total_samples=50000)
    # robust_train_generate(hp, args, audio, test_spk, total_samples=100)